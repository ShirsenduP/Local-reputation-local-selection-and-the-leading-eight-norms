...
if __name__ == '__main__':
    ...
    if reRunSimulations:
        ...
        print(default)
        print()
        E = Experiment(networkType=GrGeERNetwork, defaultConfig=default, variable='population', values=popsVsAllD,
                       repeats=reps)
        # E.run(display=True)

        default2 = Config(densities=0.1)
        E2 = Experiment(networkType=LrGeERNetwork, defaultConfig=default2, variable='population', values=popsVsAllD,
                        repeats=reps)
        E2.run(display=True)

        default3 = Config(densities=0.1)
        E3 = Experiment(networkType=LrLeERNetwork, defaultConfig=default3, variable='population', values=popsVsAllD,
                        repeats=reps)
        E3.run(display=True)
    ...
    #     E4 = Experiment(networkType=GrGeERNetwork, defaultConfig=default4, variable='population', values=pops,
    #                     repeats=reps)
    #     E4.run(display=True)
    #
