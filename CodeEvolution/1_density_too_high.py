...
if __name__ == '__main__':
    ...
    if reRunSimulations:
        ...
        E = Experiment(networkType=GrGe_Network, defaultConfig=default, variable='population', values=pops,
                       repeats=reps)
        E.run(export=True)

        default2 = Config(densities=0.1)
        E2 = Experiment(networkType=LrGe_Network, defaultConfig=default2, variable='population', values=pops,
                        repeats=reps)
        E2.run(export=True)

        default3 = Config(densities=0.1)
        E3 = Experiment(networkType=LrLe_Network, defaultConfig=default3, variable='population', values=pops,
                        repeats=reps)
        E3.run(export=True)
